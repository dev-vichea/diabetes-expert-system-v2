  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 bg-white/95 p-3.5 shadow-xl backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/95">
      <div className="flex items-center justify-between gap-3">
        ...
        >
          {isInRange ? '✓ In Target' : isHigh ? 'Above Target' : 'Below Target'}
        </span>
      </div>
      ...
      <p className="mt-1 text-[10px] font-medium text-slate-400">
        Clinical Range: {TARGET_MIN}–{TARGET_MAX} mg/dL
      </p>
    </div>
  )
}
...
export function PatientGlucoseTrend({ results }) {
  ...
  return (
    <div className="flex h-full flex-col justify-between rounded-[28px] border border-slate-200/70 bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.03)] dark:border-slate-800 dark:bg-slate-900 sm:p-7">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Compared with your clinical target zone ({TARGET_MIN}–{TARGET_MAX} mg/dL)
            </p>
          </div>

          {/* Target Zone Badge */}
          <div className="flex items-center gap-2 text-xs">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1 font-semibold text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300">
              <span className="h-2 w-2 rounded-full bg-emerald-500" />
              <span>Target Zone: 80–130</span>
            </span>
          </div>
        </div>

        {/* Minimalist Apple Health Chart */}
        <div className="mt-6 h-56 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 12, right: 12, left: -20, bottom: 0 }}>
              <defs>
                <linearGradient id="appleGlucoseGrad" x1="0" y1="0" x2="0" y2="1">
      ...
      {/* Apple Health KPI Pills */}
      <div className="mt-5 grid grid-cols-3 gap-2.5 rounded-2xl bg-[#fafafc] p-3 text-center dark:bg-slate-800/40">
        <div>
