...
const SidebarGroupLabel = React.forwardRef(({ className, ...props }, ref) => (
  <p
    ...
    className={cn('px-3 pb-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400', className)}
    {...props}
  />
))
...
const SidebarMenuButton = React.forwardRef(({ className, asChild = false, children, ...props }, ref) => {
  const baseClassName = cn(
    'flex w-full items-center gap-2 rounded-2xl px-3 py-2.5 text-left text-sm font-medium text-slate-600 transition-colors duration-200 hover:bg-primary-50 hover:text-primary-800 dark:text-slate-400 dark:hover:bg-[#181830] dark:hover:text-slate-100',
    className
  )
...
const SidebarSeparator = React.forwardRef(({ className, ...props }, ref) => (
  <div ref={ref} data-slot="sidebar-separator" className={cn('mx-2 my-2 h-px bg-slate-200 dark:bg-[#1e2234]', className)} {...props} />
))
