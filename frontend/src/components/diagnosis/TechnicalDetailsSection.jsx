...
export function TechnicalDetailsSection({ children, embedded = false }) {
  ...
  if (embedded) {
    return (
      <div className="my-6 overflow-hidden rounded-2xl bg-slate-100/70 p-4 sm:p-6 dark:bg-slate-900/50">
        <button
          type="button"
        ...
        >
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-slate-200/80 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-slate-200/80 px-2 py-0.5 text-[10px] font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                  Clinician / EMR
                </span>
              </div>
        ...
        {open ? (
          <div className="mt-4 space-y-5 rounded-xl bg-white/80 p-4 sm:p-6 dark:bg-slate-950/60 border border-slate-200/60 dark:border-slate-800">
  ...
  return (
    <section className="my-6 space-y-4 rounded-2xl bg-slate-100/70 p-4 sm:p-6 dark:bg-slate-900/50">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          onClick={() => setOpen((value) => !value)}
          aria-expanded={open}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white transition-colors"
        >
      ...
      {open ? (
        <div className="mt-4 space-y-6 rounded-xl bg-white/80 p-4 sm:p-6 dark:bg-slate-950/60 border border-slate-200/60 dark:border-slate-800">
          <div className="flex items-start gap-2.5 pb-2">
            <Info className="mt-0.5 h-4 w-4 shrink-0 text-slate-400 dark:text-slate-500" />
            <p className="text-xs sm:text-sm leading-relaxed text-slate-500 dark:text-slate-400">
              {t('diagnosisResult.technical.plainNote', 'In simple words: the system compared the answers with known medical rules. Every rule that matched adds a small piece of evidence, and complete lab results make the conclusion more certain. Your doctor can read this section to double-check exactly why the system reached its conclusion.')}
