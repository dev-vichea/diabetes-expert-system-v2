...
export function NotFoundPage({ isAuthenticated = false }) {
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col justify-between overflow-x-hidden bg-slate-50/80 text-slate-900 transition-colors duration-200 dark:bg-[#060814] dark:text-slate-100">
      ...
      <div className="pointer-events-none absolute -top-40 left-1/2 -z-10 h-96 w-[36rem] -translate-x-1/2 rounded-full bg-primary-500/10 blur-3xl dark:bg-primary-500/15" />
      <div className="pointer-events-none absolute -bottom-40 left-10 -z-10 h-80 w-80 rounded-full bg-cyan-500/10 blur-3xl dark:bg-cyan-500/15" />
      ...
      <main className="flex flex-1 items-center justify-center px-4 py-8">
        <div className="relative w-full max-w-xl rounded-3xl border border-slate-200/80 bg-white/90 p-8 text-center shadow-2xl shadow-slate-200/50 backdrop-blur-xl transition-all sm:p-10 dark:border-[#1e2544] dark:bg-[#0d122c]/90 dark:shadow-none">
          ...
          <div className="relative mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-3xl bg-primary-50 shadow-inner ring-8 ring-primary-50/60 transition-transform duration-300 hover:scale-105 dark:bg-primary-950/40 dark:ring-primary-950/30">
            <Compass className="h-10 w-10 text-primary-600 animate-spin-slow dark:text-primary-400" />
            <div className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-white shadow-sm ring-2 ring-primary-100 dark:bg-[#151b3a] dark:ring-primary-900/60">
              <FileQuestion className="h-3.5 w-3.5 text-primary-500" />
            </div>
          </div>

          {/* Error Tag */}
          <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary-200 bg-primary-50/80 px-3.5 py-1 text-xs font-semibold text-primary-700 dark:border-primary-900/50 dark:bg-primary-950/50 dark:text-primary-300">
            <span className="h-1.5 w-1.5 rounded-full bg-primary-500 animate-pulse" />
            <span>{t('publicPages.notFound.badge', 'Error 404')}</span>
            <span className="text-primary-300 dark:text-primary-700">•</span>
            <span>{t('publicPages.notFound.eyebrow', 'Navigation')}</span>
          </div>

          {/* Heading */}
          <h1 className="mb-3 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl dark:text-white">
            {t('publicPages.notFound.title', 'Page Not Found')}
          </h1>

          {/* Description */}
          <p className="mx-auto max-w-md text-sm leading-relaxed text-slate-600 sm:text-base dark:text-slate-300">
            {t(
              'publicPages.notFound.description',
          ...
          <div className="my-6 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 text-left text-xs text-slate-600 dark:border-[#1e2544] dark:bg-[#111736]/70 dark:text-slate-400">
            <p className="leading-relaxed">
              {t(
                'publicPages.notFound.helpNotice',
                'Please double-check the URL or return to your clinical dashboard.'
              )}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
            {isAuthenticated ? (
              ...
              >
                ...
                <span>{t('publicPages.notFound.backDashboard', 'Back to Dashboard')}</span>
              </Link>
            ) : (
              <Link
                to="/"
              ...
              >
                ...
                <span>{t('publicPages.notFound.home', 'Home')}</span>
              </Link>
            )}
            ...
            >
              ...
              <span>{t('publicPages.notFound.goBack', 'Previous Page')}</span>
            </button>
            ...
            {!isAuthenticated && (
              ...
              >
                <span>{t('publicPages.notFound.goLogin', 'Go to Login')}</span>
              </Link>
            )}
          </div>
        </div>
